...
/// belt slot or sell it to the nearest trader for rubles.
/// </summary>
public sealed class ArtifactDecisionEngine
{
    ...
    // Rubles threshold below which a stalker is cash-hungry (sell bias)
    public float RubleDesperationMark   { get; set; } = 500f;

    /// <summary>
    /// Solve the Keep vs. Sell decision.
    /// High rarity = equip. Low rubles = sell. Otherwise stash.
    /// </summary>
    public ArtifactDecision Decide(ArtifactData artifact, SurvivalNeeds needs, bool hasFreeSlot)
    {
        ...
        if (needs.Rubles < RubleDesperationMark && artifact.RarityScore < 0.85f)
